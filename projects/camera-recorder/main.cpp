...
const int KEY_EXIT = 113;  // keyboard q
...
void Usage() {
  std::cout << "./Mp4Recorder device_id save_path max_slice_time(unit:min) fps "
               "width height"
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  if (mp4_name.rfind("/") != std::string::npos) {  // path is unix style
    mp4_name += "/";
  } else if (mp4_name.rfind("\\") !=
             std::string::npos) {  // path is windows style
  ...
  bool status =
      write.open(mp4_name, CV_FOURCC('H', '2', '6', '4'), fps, dst_size, true);
  if (!status) {
    std::cout << "video writer open " << mp4_name << " failed!!, fps = " << fps
              << std::endl;
    capture.release();
    write.release();
    getchar();
  ...
  while (running) {
    ...
    if (src_size != dst_size) cv::resize(frame, dstframe, dst_size);
    write << dstframe;
    ...
    if (cur_time - history_time >= slice_time) {
      ...
      mp4_name += TimeMstoStringMs(GetEpochTimeMs());
      mp4_name += ".mp4";
      status = write.open(mp4_name, CV_FOURCC('H', '2', '6', '4'), fps,
                          dst_size, true);
      if (!status) {
        std::cout << "video writer open " << mp4_name
                  << " failed!!, fps = " << fps << std::endl;
        capture.release();
        write.release();
        getchar();
        return -1;
